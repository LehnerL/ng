...
export class UserManager {
    ...
    private static loadFill():void {
        if (localStorage.getItem(USER_CREDENTIALS_KY) == null) {
            ...
            try {
                let userKey:any = localStorage.getItem(USER_CREDENTIALS_KY);
                UserManager.theUserCredentials = JSON.parse(userKey);
            }
            catch (prblm: any) {
                localStorage.setItem(USER_CREDENTIALS_KY, JSON.stringify(UserManager.theUserCredentials));
    ...
    static showErrorFocus(msg: string, id: string): void {
        alert(msg);
